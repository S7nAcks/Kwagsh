...
# Kwagsh Multi-installer
# a one line clone-and-compile for kwagsh:
#
#     ` $ curl -sL "http://kwagsh.xyz/files/compile/multi-installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/kwagsh ]; then
            read -r -p "${1:-kwagsh directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old kwagsh directory" && echo
                    rm -rf "$PWD"/kwagsh
                    ;;
                *)
                    _fail "kwagsh directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir kwagsh
        git clone -q https://github.com/kwash-dev/Kwagsh kwagsh   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd kwagsh
    fi
}

_build_kwagsh() {
    _note "Building kwagsh from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/kwash-dev/')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/kwash-dev/')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "Kwagsh Multi_Installer"
...
_note "Look in 'kwagsh/build/src/' for the executible binaries. See 'https://github.com/kwash-dev/Kwagsh' for more project support."
